...
interface PrologueImage {
    src: string;
    title: string;
    category: string;
}

const PROLOGUE_IMAGES: PrologueImage[] = [
    {
    ...
    },
    {
        ...
        category: "ARQUITECTURA HISTÓRICA PATRIMONIAL"
    },
    {
...
export default function PrologoGallery() {
    const [selectedImage, setSelectedImage] = useState(PROLOGUE_IMAGES[0]);

    return (
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 w-full items-stretch">
            {/* Main Image Container (with maintained layout and design) */}
            <div className="md:col-span-8 relative rounded-2xl overflow-hidden shadow-2xl group border border-[#C5A059]/30 min-h-[400px]">
                <img
                ...
                <div className="absolute bottom-6 left-6 right-6 flex items-center gap-4 text-white">
                    ...
                    <div>
                        <h4 className="font-serif text-xl sm:text-2xl font-bold tracking-wide">{selectedImage.title}</h4>
            ...
            {/* Sidebar List of Images (al lado) */}
